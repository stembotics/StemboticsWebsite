/* empty css                                 */
import { c as createComponent, a as createAstro, e as renderComponent, r as renderScript, d as renderTemplate, m as maybeRenderHead } from '../chunks/astro/server_BXRAwln_.mjs';
import 'kleur/colors';
...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  `,
    args: [user?.userId || null, user?.userId || null]
  });
  const courses = result.rows.map((row) => {
    const courseId = String(row.id);
    const isLiked = user ? likedCourses.includes(courseId) : false;
    return {
      id: courseId,
      title: String(row.title),
      description: String(row.description),
      ...
      instructor: {
        ...
        role: String(row.instructor_role)
      },
